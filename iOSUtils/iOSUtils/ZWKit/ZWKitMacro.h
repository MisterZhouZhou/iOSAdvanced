...
static inline NSDate *_ZWCompileTime(const char *data, const char *time) {
    ...
    [formatter setDateFormat:@"MMM dd ZWZW HH:mm:ss"];
...
#define ZWMUTEX_ASSERT_ON_ERROR(x_) do { \
__unused volatile int res = (x_); \
assert(res == 0); \
} while (0)
    assert(mutex != NULL);
    if (!recursive) {
        ZWMUTEX_ASSERT_ON_ERROR(pthread_mutex_init(mutex, NULL));
    } else {
        pthread_mutexattr_t attr;
        ZWMUTEX_ASSERT_ON_ERROR(pthread_mutexattr_init (&attr));
        ZWMUTEX_ASSERT_ON_ERROR(pthread_mutexattr_settype (&attr, PTHREAD_MUTEX_RECURSIVE));
        ZWMUTEX_ASSERT_ON_ERROR(pthread_mutex_init (mutex, &attr));
        ZWMUTEX_ASSERT_ON_ERROR(pthread_mutexattr_destroy (&attr));
    }
#undef ZWMUTEX_ASSERT_ON_ERROR
}
